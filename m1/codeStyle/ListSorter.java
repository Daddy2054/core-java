// Source: https://github.com/AllenDowney/ThinkDataStructures
// MIT license
...
public class ListSorter<T> {
  ...
  public void insertionSort(List<T> list, Comparator<T> comparator) {
    for (int i = 1; i < list.size(); i++) {
      ...
      int j = i;
      while (j > 0) {
        T valueOfj = list.get(j - 1);
        if (comparator.compare(valueOfi, valueOfj) >= 0) {
          break;
        }
        list.set(j, valueOfj);
        j--;
  ...
  public static void main(String[] args) {
    ...
    ListSorter<Integer> s = new ListSorter<Integer>();
    s.insertionSort(list, comparator);
    System.out.println(list);

    list = new ArrayList<Integer>(Arrays.asList(3, 5, 1, 4, 2));
    s.mergeSortInPlace(list, comparator);
    System.out.println(list);

    list = new ArrayList<Integer>(Arrays.asList(3, 5, 1, 4, 2));
    s.heapSort(list, comparator);
    System.out.println(list);

    list = new ArrayList<Integer>(Arrays.asList(6, 3, 5, 8, 1, 4, 2, 7));
    List<Integer> queue = s.topK(4, list, comparator);
